  ...
  >
    ...
    <div v-if="tempLocation" class="mt-4 p-3 bg-gray-50 rounded-lg">
      ...
      <div v-else class="flex items-start gap-2">
        ...
        <div class="flex-1">
          <p class="font-medium text-sm">{{ tempLocation.namaLokasi || tempLocation.address || 'Lokasi dipilih' }}</p>
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      if (formData.value.kecamatan_id) {
        selectedKecamatan.value = kecamatanList.value.find(k => k.id === formData.value.kecamatan_id) || null
    ...
    const err = error || {}
    const errorMessage = (err.response && err.response.message) || err.message || 'Gagal memuat kecamatan'
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: errorMessage,
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      if (formData.value.desa_id) {
        selectedKelurahan.value = kelurahanList.value.find(k => k.id === formData.value.desa_id) || null
      }
    }
  } catch (error) {
    const err = error || {}
    const errorMessage = (err.response && err.response.message) || err.message || 'Gagal memuat desa/kelurahan'
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: errorMessage,
...
const reverseGeocode = async (lat, lng) => {
  ...
  try {
    ...
    if (data) {
      ...
      const namaLokasi = addressPartsArray[0]?.trim() || `Lokasi Aduan (${lat.toFixed(6)}, ${lng.toFixed(6)})`
