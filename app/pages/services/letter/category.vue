<template>
  <div class="flex justify-between h-full overflow-y-auto flex-col">
    <div class="space-y-6 flex-1 mb-5">
      <h1 class="font-semibold text-xl text-gray-800">Pilih Jenis Surat</h1>
    ...
    <div class="flex items-center mb-5 gap-3">
      <Button
        ...
        class="shrink-0"
      />
      <NuxtLink :to="selectedCategory ? `/services/letter/form?jenis=${selectedCategory}` : '#'" class="w-full">
        <Button
          label="Selanjutnya"
...
const fetchKategori = async () => {
  ...
  try {
    ...
    const err = error || {}
    const errorMessage = (err.response && err.response.message) || err.message || 'Gagal memuat kategori surat'
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: errorMessage,
